...
function UserSockets(sio, sessionStore) {
  ...
  sio.adapter(mongo(config.db_uri));
...
UserSockets.prototype =  {
  ...
  send: function (users, args) {
    this.emit(args);
    //users.forEach(user => {
    //  if (!this.sockets[user]) return;
    //  this.sockets[user].emit(args);
    //});
  },
  ...
  sendEach: function (users, fn) {
    users.forEach(user => {
      if (!this.sockets[user] || !this.sockets[user].user) return;
      this.emit(fn(this.sockets[user].user));
    });
  },
  ...
  emit: function (args) {
    this.sio.sockets.emit.apply(this.sio.sockets, args);
  }
};
