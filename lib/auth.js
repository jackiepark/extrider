...
var GITLAB_CLIENT_SECRET = "";
var GITLAB_CALLBACK_URL = "";
...
var setupGitlabAuth = function () {
  passport.use(new GitlabStrategy({
      ...
      console.log('setupGitlabAuth');
      User.findOne({ email: profile.emails[0].value }, function(err, user) {
        if(err) {
          console.log(err);  // handle errors!
          return done(null, false, { message: err.toSting() });
        }
        if (!err && user !== null) {
          done(null, user);
        } else {
          User.registerWithGitlab(profile.emails[0].value, profile.username, function (err, user) {
            if (err || !user) {
              console.log(err);
            }
...
var _authenticate = passport.authenticate('gitlab', {
  successRedirect: '/',
